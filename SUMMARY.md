...
> **Canonical state doc**: [`docs/handovers/v0.6.2-restart-roadmap-2026-08-21.md`](docs/handovers/v0.6.2-restart-roadmap-2026-08-21.md). If this section and that doc ever disagree, the handover wins (CLAUDE.md rule #6 — state lives in one place).
...
- ⚠️ **Known-red CI**: the `test.yml` pytest job has been failing on `main` since 2026-06-22 (60 consecutive runs checked, through the latest commit 2026-08-19). ~60 assertion failures, mostly UI-contract tests that did not follow the page rebuilds, plus a genuine rule #5 module-size violation (`core/response_style.py`, `core/reasoning/engine.py`). The deterministic benchmark tier (`bash benchmarks/run_all.sh`) is unaffected. Restoring green is Phase 2 of the restart roadmap.
...
| Know the current state / what to work on next | `docs/handovers/v0.6.2-restart-roadmap-2026-08-21.md` (restart roadmap, Phase 1-7) |
