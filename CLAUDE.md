...
| Purpose | File |
|---|---|
| **🟢 NEXT SESSION ENTRY (read this first)** | **`docs/handovers/v0.4-cycle-gamma-phase-a-closure-and-phase-b-launch-2026-06-08.md`** (BIG: cycle γ Phase A 완전 closed = 10 PRs #724-#733 + Phase B 진입 = 3 PRs #734-#736. **⭐⭐ candidate finding (operator review)**: raw gemma4:e4b RGB-en negative_rejection_f1 = 0.000 (TP=0/FN=25, 100% hallucinate on no-evidence). 다음 = JAMES engine producer 같은 25 negrej 재측정 → cycle γ design memo §2 abstention 가치 주장 실증 직접 evidence. **의무 reading**: 5 production-safety notes + decision tree (Option A JAMES engine / B 다른 벤치 다운로드 / C handover-only). PR convention 위반 1건 (`chore: .gitignore` main 직접 push) 기록.) |
...
**현재 위치 (2026-06-08)**: **Cycle γ Phase A 완전 closed (10 PRs #724-#733) + Phase B 진입 (3 PRs #734-#736).** 4 외부 벤치 (RGB / ALCE / MuSiQue / 2Wiki) loader + scorer + 통합 runner 완성, `python scripts/external_bench_run.py --bench <name> --mode {closed-corpus,james} --model gemma4:e4b` 한 줄로 가동. 첫 라이브 smoke 가 production wiring bug 2개 (JSONL parse / 4000자 truncation cap) + scope gap 1개 (RGB negrej 측정 가능성) catch — `feedback_measurement_smoke_caught_wiring_bugs` 룰 3회 실증. **⭐⭐ candidate finding (operator review 대기)**: raw gemma4:e4b RGB-en n=50 dual-axis baseline = noise_robustness 0.600 / **negative_rejection_f1 0.000** (TP=0/FN=25, **25개 negrej query 100% hallucinate**). 같은 25 negrej 를 JAMES engine producer (full stack abstention layer) 로 재측정하면 cycle γ design memo §2 abstention 가치 주장의 직접 실증 evidence (요구: RGB workspace corpus 색인 ~30-60분 운영자 작업). **다음 자연 옵션 A/B/C** (handover §4 decision tree). Path 1 sequence: cycle β #1+#1.5+#2+#6+#7 ✅ → #3 skip (self-eval 함정) → v0.4.2 T5 ✅ → **cycle γ Phase A ✅ + Phase B 부분 진행 (이 세션)** → Phase C/D → v0.5 진입. PR convention 위반 1건 (`chore: .gitignore` main 직접 push, commit `ff74243`) 기록 — 향후 PR 패턴 유지. v0.5 도메인 (기업 내부지식, 수평) 여전히 gated. 자세한 내용은 `docs/handovers/v0.4-cycle-gamma-phase-a-closure-and-phase-b-launch-2026-06-08.md` + memory `project_cycle_gamma_phase_a_closed`.
