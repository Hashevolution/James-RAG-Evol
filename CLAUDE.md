...
| Purpose | File |
|---|---|
| **🟢🟢 NEXT SESSION ENTRY (read this first — BIG)** | **`docs/handovers/v0.4-cycle-gamma-phase-c-4model-true-signal-2026-06-08.md`** (Phase C 4-model 완료, 200 LLM 호출. **⭐⭐ confirmed ★재정정★**: inverse-lift 가설 mxtral 에서 ex-ante prediction 실패로 FALSIFIED → 더 강한 **"true signal" mechanism** = mxtral JAMES set ≡ llama JAMES set {3,9,14,15,17,18} EXACTLY (다른 architecture/family/training 임에도). JAMES = benchmark-level latent abstention signal emitter, 모델 capacity 가 absorbed fraction 결정. CASCADE-class 4/4 통과. **사용자 catches 3건** (post-hoc fit vs prediction / 0.000 진위 / family vs size) 이 falsification 견인. 의무 reading: §3 falsification narrative + §4 true-signal mechanism + §5 honest framing tier + §8 next-session decision tree (권장: Phase B Option B cross-bench → ⭐⭐⭐ 격상 직접 경로). 0.000 audit script `scripts/research/_audit_gemma4_negrej.py` 보존.) |
...
**현재 위치 (2026-06-08, BIG 세션)**: **Cycle γ Phase A + Phase B + Phase C 4-model RGB-en 완료. ⭐⭐ confirmed ★재정정★ — Inverse-lift 가설 mxtral falsified → "true signal" mechanism 으로 강화**. Phase A scaffolding 10 PRs + Phase B 3 PRs + Option A wiring 1 PR + handovers 2 PRs = 16 PRs. Phase B+C 측정 = 200 LLM 호출 (4 모델 × 50 query × 2 modes). **핵심 발견**: mxtral JAMES abstention set = llama JAMES set EXACTLY {3,9,14,15,17,18} (47B MoE vs 8B dense, 다른 family/training 임에도 IDENTICAL). JAMES 가 benchmark-level "true signal" 발산, 모델 capacity 가 absorbed fraction 결정 (gemma4 4/6, gemma3:12b 3/6, mxtral 6/6, llama 6/6). CASCADE-class 4/4 통과. **사용자 3건 catch** (post-hoc fit vs prediction → `feedback_finding_size_honest_framing` / 0.000 진위 audit → real measurement 확인 / family vs size separation → mxtral 측정 동기). Phase B 라이브 smoke 3회 catch (JSONL / 4000자 cap / RGB negrej scope) = `feedback_measurement_smoke_caught_wiring_bugs` 룰 3회 실증. Path 1: cycle β ✅ → v0.4.2 T5 ✅ → **cycle γ Phase A+B+C ✅** → Phase B Option B cross-bench (ALCE/MuSiQue/2Wiki, operator 다운로드) → ⭐⭐⭐ universal-law 후보 격상 → v0.5 진입. PR convention 위반 1건 (chore main 직접 push, commit `ff74243`) 기록. v0.5 도메인 여전히 gated. 자세한 내용 = `docs/handovers/v0.4-cycle-gamma-phase-c-4model-true-signal-2026-06-08.md` + memory `project_cycle_gamma_phase_b_rgb_baseline` (Phase C 섹션 추가됨).
