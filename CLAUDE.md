...
| Purpose | File |
|---|---|
| **🟢🟢🟢 NEXT SESSION ENTRY (read this first — BIG)** | **`memory/feedback_d2_v2_softener_bilingual_regression.md`** (D2 V2 측정 negative evidence + 8번째 catch. JAMES = cross-lingual 시스템 — Korean-only 결론 X. V2 단순 영어 trigger 확장 (BILINGUAL=1) F1 -0.111 regression, "respond naturally" retry 가 영어 모델 destabilize. Path D 유지하면서 abst_f1 lever 큰 곳 = D1 retrieval (Phase D 가 5/6 contribution 측정). 다음 세션 자연 시작 = D1 design (어떤 sub-option 부터: reranker / embedding / chunk strategy). PR #746 D2 V2 코드 + tests + measurement 영구 기록.) |
| **🟢🟢 Path D positioning (7번째 catch)** | **`memory/feedback_path_d_james_not_specialty_verifier.md`** (JAMES ≠ HALT-RAG specialty verifier. abst_f1 단일 axis 추격 거부.) |
