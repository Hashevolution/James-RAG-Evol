...
| Purpose | File |
|---|---|
| **🟢🟢🟢 NEXT SESSION ENTRY (read this first — BIG)** | **`memory/feedback_path_d_james_not_specialty_verifier.md`** (Path D positioning 결정 7번째 catch. JAMES = full-RAG + replayable audit 카테고리, HALT-RAG = post-hoc verification specialty 카테고리. abst_f1 단일 axis 추격 거부 = Path A/B/C 모두 reject. 미래 세션이 "JAMES abst_f1 약함 → NLI 통합" framing 시작하면 reject template 사용. 동반 reading: prior-art positioning §5 Path D subsection + Phase D handover + axis-misalignment rule.) |
