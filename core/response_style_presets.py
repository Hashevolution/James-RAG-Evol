"""Response-style presets — StylePreset dataclass + the three presets.

Extracted from ``core/response_style.py`` during the 2026-07-01
module-size split (CLAUDE.md rule #5 — the file crossed the 20 KB cap
when DETAILED_PRESET landed in PR #1074). Behaviour is byte-identical
to the pre-split file; only the location moved. ``core/response_style``
re-exports everything here, so all existing import sites
(``from core.response_style import NATURAL_PRESET`` etc.) keep working.

See ``core/response_style.py`` for the module-level design history
(v1 token-cut presets → v2 NATURAL redesign → terse v4 → detailed).
"""
