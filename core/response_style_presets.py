"""Style presets — the `StylePreset` dataclass and the three instances.

Split out of ``core/response_style.py`` on 2026-08-26: that file had
grown to 22,036 bytes, over CLAUDE.md rule #5's 20 KB ceiling, and the
three preset bodies were 12 KB of it. This module holds the data; the
resolver and the public API stay in ``core/response_style.py``, which
re-exports everything here so every existing
``from core.response_style import ...`` keeps working unchanged.

Import direction is one-way — this module must NOT import
``core.response_style`` — so the two cannot form a cycle.
"""
