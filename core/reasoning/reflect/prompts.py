...
REVISE_PROMPT_KO = (
    ...
    "- 검토가 어떤 주장을 '자료 미근거' 로 지적했다면 그 주장을 "
    "**삭제하라.** 출처를 덧붙여 정당화하지 마라 — 근거가 없다는 지적에 "
    "인용을 붙이는 것은 수정이 아니다.\n"
    "- 삭제하고 나면 질문에 답할 근거가 남지 않는 경우, 자료에 답이 "
    "없다고 밝혀라. 추측으로 빈자리를 채우지 마라.\n"
    "- 의미를 보존하고 새 사실을 만들지 마라 (단, 위 삭제 규칙이 우선).\n"
...
REVISE_PROMPT_EN = (
    ...
    "- If the review flags a claim as unsupported by the evidence, "
    "**remove it.** Do not keep it and attach a citation — answering "
    "'this is not supported' with a source reference is not a fix.\n"
    "- If removing it leaves the question unanswered, say the evidence "
    "does not contain the answer. Do not fill the gap with a guess.\n"
    "- Preserve meaning; don't invent new facts (the removal rule "
