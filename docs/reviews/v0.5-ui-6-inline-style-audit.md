...
## 9. Update 2026-06-25 — HTML surface migrated (Option B chosen)

**Status**: HTML inline-style surface DONE via Option B (class
migration). CSP **not yet flipped to enforce** — the JS-injected
inline-style surface remains.

### 9.1 §4.1 correction — nonces do NOT cover inline `style="..."`

The §4.1 "Option A per-request nonce (권장)" framing was **wrong for
inline `style="..."` attributes**. CSP nonces apply only to `<style>`
and `<script>` BLOCK elements (CSP3 §6.6); an inline `style=""`
attribute can never be nonce-bound. Since the pages carry **zero**
`<style>` blocks (§2) and all the remaining styling is in attributes,
the nonce path buys nothing for `style-src`. The correct — and only —
path is **Option B (class migration)**. (The `core/security/csp_nonce.py`
module docstring already states this in "What this module is NOT"; the
audit §4.1 recommendation predates that and is hereby corrected.)

### 9.2 What landed

`scripts/migrate_inline_styles.py` (deterministic, re-runnable)
relocated **596 inline `style="..."` attributes** across the 5 served
pages (index / admin / graph / workspace / intro) into CSS classes in
`tokens.css`:

  * **103 atom utilities** (reusable single-property classes:
    `d-flex`, `gap-8`, `mt-12`, `c-muted`, `fs-11`, …) for the common
    compositional one-liners, and
  * **184 verbatim "component" classes** (`.u-<hash>`) that hold a
    one-off multi-declaration blob unchanged — a lossless fallback, so
    the migration can never silently drop a declaration.

`display:none` → `.d-none` **without `!important`** so JS toggling
`el.style.display` still overrides it. The one break pattern —
`el.style.display = ''` (clear-to-show) which a class cannot defeat —
was fixed at its 9 migrated call-sites by adding
`classList.remove('d-none')` (workspace-badge / templating-chat /
workspace / upload / graph_editor). Completeness argument: an element
hidden by an *inline* `display:none` could only ever have been shown by
an *inline* `style.display` write (a class rule can't beat inline), so
the show surface is exhaustively {explicit value → overrides `.d-none`}
∪ {`''` clear → the 9 fixed sites}.

Guard: `tests/test_v06_csp_inline_style_migration.py` pins zero inline
`style=` on the 5 pages + the generated block + `.d-none` non-important.

### 9.3 Remaining surface before `style-src` can enforce

Enforce is still gated — the JS-injected inline-style surface persists:

| Surface | Count | Technique to remove |
|---|---|---|
| `style=` in JS `innerHTML` templates | ~482 | classes; **dynamic values** (`width:${pct}%`) need CSSOM `el.style.setProperty()` refactor, not a class |
| `.style.cssText=` / `setAttribute('style')` | ~11 | same CSSOM refactor |

These need live verification (KG-mutating + non-deterministic render
paths), so they are a separate follow-up. `JAMES_CSP_MODE` stays
`report-only` and `style-src` keeps `'unsafe-inline'` until that lands.
Operator runs `python scripts/visual_regression.py` (server up) to
confirm this HTML migration is pixel-identical before the next step.
