...
| **D-2** | Chat-side per-call confirm card (`Allow / Deny / Always-allow`) + streaming output + cancel button | Partial — session-scoped "auto-allow" checkbox shipped (#921); per-call interactive confirm still deferred (needs SSE/WS) |
| **E** | `run_shell` tool (`core/agent_tools/shell.py`) + wider `RUN_SHELL_EXTRA_BLOCKED` list on top of sandbox `BLOCKED_COMMANDS` + powershell/pwsh/cmd/bash/sh dispatch (explicit argv, no `shell=True` splat) + `SHELL_TIMEOUT_SEC`=20 + 4 KB output cap + audit | ✅ THIS PR — ships **default OFF** behind `JAMES_AGENT_ENABLE_SHELL=1` (mirrors the AnthropicBackend `JAMES_AGENT_ALLOW_CLOUD` opt-in). Hidden from the LLM schema while disabled. admin-only; `cwd` must pass `validate_path` (workspace or operator-registered folder). Per-call interactive confirm (D-2) remains the next hardening step before opening to non-admin roles. |
...
- ~~No automatic path discovery / autocomplete~~ **(revised — UX overhaul,
  operator-requested)**: a read-only **server-side folder browser**
  (`GET /admin/agent/browse`) now lets the operator navigate the host's
  directories and click a folder to register it instead of hand-typing
  an absolute path. It is admin-only, lists directories only, and marks
  non-registerable folders (critical roots / repo-protected) so the
  "select" button is disabled there — the selection is still an explicit
  operator act, not silent auto-discovery.

## 8a. UX overhaul (v0.6.1, operator-requested) — shipped

Four operator-requested ergonomics on top of Phases B–E:

1. **Folder picker** — `GET /admin/agent/browse` server-side directory
   browser (drives / home → subfolders), feeding the existing
   `register_user_path` POST.
2. **LLM model selection in the UI** — `GET/POST /admin/agent/llm-settings`
   reads the installed Ollama models (reuses
   `routes/llm.py::_list_installed_ollama_models`) and persists the
   agent backend + model to the unified `core/llm_settings` repo; the
   chat endpoint also takes a per-turn `model` override
   (`get_backend(name, model)`).
3. **Merged page** — the separate "에이전트 폴더" + "에이전트 챗" admin
   pages are now one **에이전트** page (session sidebar + control card
   with backend/model + collapsible folder panel + chat).
4. **Separate conversations** — server-persisted sessions
   (`core/agent_sessions.py` + `routes/agent_sessions.py`, SQLite in
   `james_data.db`); each conversation has its own history, survives
   restart, and is auditable (operator-chosen over client localStorage).

Tests: `tests/test_v06_agent_ux.py`.
