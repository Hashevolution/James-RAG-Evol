...
> **정정 (2026-09-06, 발송 후)**: 본문의 *"seven consumers"* 는 틀렸다.
> `core.i18n` 소비 모듈은 **6개**, 호출 지점은 8곳이며, 이 편지를 쓴
> 커밋(`6d6a079`)에서도 6개였다. **발송본이므로 본문은 고치지 않는다.**
> 정정은 ⑤(`finding-5-isolate-downgrade-reply.md`) 본문에서 Ali 에게
> 직접 전달했다 — 그가 5차에서 이 숫자를 인용해 되돌려줬기 때문이다.
> 경위는 README "3차 — 발송 후 발견된 오류" 참조.
