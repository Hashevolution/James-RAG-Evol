...
**⑤는 이 순서에 끼지 않는다.** Ali 5차(2026-09-06)는 회신이 아니라
반대 방향 감사이고, 그가 *"a question, not a finding"* 으로 명시해 남긴
isolate 질문에 우리 측정이 나왔다. ④의 재측정을 기다릴 이유가 없다 —
그가 말한 *"no clock on this side"* 는 ④에 대한 것이다. 게다가 그는
자기 numeral/orthography 수정이 미출시 상태(*"still live defects"*)라고
밝혔으므로, "isolate 쪽은 막을 필요 없다"는 결과는 그의 큐 배분에
쓰이는 정보다. 늦을수록 가치가 준다.
...
## 3차 — 발송 후 발견된 오류 1건 (⑤ 작성 중)

⑤ 편지가 ③의 문장을 다시 쓰려다 걸렸다. **재현되지 않는 숫자를 다시
쓰지 않는다**는 규칙이 잡아낸 첫 사례다.

| | 내용 |
|---|---|
| ③ | *"the detector, **seven** consumers, three tokenisers, ..."* — `core.i18n` 소비 모듈은 **6개**(planner / query_rewriter / verify / reflect.loop / engine_memory / engine_synth), 호출 지점은 8곳. 어느 쪽도 7이 아니다. 편지 작성 시점 커밋(`6d6a079`)에서도 6개였으므로 코드 이동이 아니라 **작성 당시의 오류**다. |

이 오류는 **이미 발송됐고**, Ali 5차가 *"Yours is a Hangul counter
feeding seven consumers"* 로 그대로 인용해 되돌려줬다. 그래서 ⑤ 본문에
정정을 넣었다 — 숫자를 조용히 6으로 바꾸면 그가 인용한 것과 어긋나고,
7을 반복하면 검증되지 않은 것을 다시 보내게 된다. 어디서 7이 나왔는지
재구성할 수 없다는 것까지 그대로 적었다. 발견의 형태는 바뀌지 않는다.

**발송된 텍스트는 고치지 않는다.** `finding-3-*.md` 와
`COMBINED-findings-1-2-3.md` 의 "seven" 은 발송본 그대로 두고, 각 파일
상단에 정정 주석만 단다. 나간 편지와 보관본이 달라지면 다음 세션이
어느 쪽을 믿을지 알 수 없다.
