...
ATOMS: Dict[str, str] = {
    ...
    "flex:1": "flex-1",
    # min-width — mobile.css collapses these to 100% on narrow
    # screens. It used to do that with [style*="min-width:220px"]
    # attribute selectors, which stop matching the moment a value
    # moves out of the style attribute, so they get real classes.
    "min-width:200px": "minw-200",
    "min-width:220px": "minw-220",
    "min-width:280px": "minw-280",
    "flex:2": "flex-2",
    # align / justify
...
# JS files whose template literals emit markup with inline styles.
# Added one at a time, each with its own verification pass — see the
# Phase 3.3 slices. A file joins this list once its dynamic sites are
...
def migrate_js(
    ...
    """Rewrite static inline styles inside JS-built markup.

    JS emits tags through template literals and string concatenation
    (``innerHTML = `<div style="...">` ``). Once ``style-src`` drops
    ``'unsafe-inline'`` those attributes are blocked exactly like the
    HTML ones — measured 2026-09-08, see the Phase 3.3 pilot (#1095).
    CSSOM writes (``el.style.x = …``, ``cssText``, ``setProperty``) are
    NOT governed by CSP, so they are deliberately left alone.

    A style value containing ``${`` is skipped: its declarations are
    computed, so it cannot become a static class. Those sites get
    converted by hand — class for the static half, CSSOM for the
    dynamic half — and are counted as ``skipped`` here so the
    remaining work stays visible instead of silently passed over.
    """
    skipped = [0]
    done = [0]
    ...
    def repl(m):
        tag = m.group(0)
        m_style = _STYLE_RE.search(tag)
        val = m_style.group(1) if m_style else ""
        # Two shapes cannot become a static class:
        #   `${...}`   — the declarations are computed at render time
        #   `' + '`    — the attribute is split across a JS string
        #                concatenation, so the regex captured operator
        #                text as if it were a declaration. Migrating it
        #                anyway emitted `.u-…{…font-size:12px;'` — a
        #                truncated rule that silently dropped the two
        #                declarations after the join (caught 2026-09-08
    ...
    new_text = _TAG_RE.sub(repl, text)
    return new_text, done[0], skipped[0]


def generate_css(used_atoms: Dict[str, None],
                 components: Dict[str, List[str]],
    ...
    # Rules from a previous run that this one did not regenerate. A
    # file migrated earlier carries no style attributes any more, so
    # this run cannot rediscover its classes; carrying them forward is
    # what keeps the block additive. See existing_block_rules().
    if carried:
        fresh = set(used_atoms) | set(components)
        keep = {k: v for k, v in carried.items() if k not in fresh}
        if keep:
            lines.append("/* carried forward from earlier runs */")
            for name in sorted(keep):
                lines.append(f".{name}{{{keep[name]}}}")
    lines.append(CSS_END)
    return "\n".join(lines) + "\n"


def existing_block_rules(css_text: str) -> Dict[str, str]:
    """Parse the rules already inside the generated block.

    The block is regenerated wholesale from what a run collected, and a
    run only collects a class when it converts a style attribute into
    it. Once a file has been migrated it has no style attributes left,
    so a later run collects nothing from it — and the regenerated block
    silently drops every class that file still references.

    Measured 2026-09-08 while adding the JS surface: re-running --apply
    took tokens.css from 296 rules to 53 and removed 266 classes the
    five HTML pages still use. The whole UI would have lost its styling
    on the next run, for any reason, with no error.

    So the block is now additive: existing rules are carried forward and
    newly generated ones are merged over them. Deleting a rule is a
    deliberate act, not a side effect of running the tool twice.
    """
    if CSS_BEGIN not in css_text or CSS_END not in css_text:
        return {}
    inner = css_text[css_text.index(CSS_BEGIN) + len(CSS_BEGIN):
                     css_text.index(CSS_END)]
    rules: Dict[str, str] = {}
    for m in re.finditer(r"^\.([A-Za-z0-9_-]+)\{([^}]*)\}", inner, re.M):
...
def main() -> int:
    ...
    carried = existing_block_rules(tokens)
    block = generate_css(used_atoms, components, carried)
    TOKENS_CSS.write_text(upsert_css_block(tokens, block), encoding="utf-8")
    print(f"\nWrote {len(rewrites)} files + CSS block in {TOKENS_CSS.relative_to(REPO)}")
    return 0
