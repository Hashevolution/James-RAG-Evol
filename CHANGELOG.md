...
- **#1123 reranker fallback accounting** — `llm_rerank.RerankStats`, per-row `rerank_fallback` in `bench.jsonl`, per-cell `rerank_fallbacks` / `rerank_fallback_rate` in `result.json`. A failed LLM call had silently kept token-overlap order; audit of every committed `bench.jsonl` by latency found one such row (mixtral vanilla, 2026-09-10) and one all-fallback June claude cell.
- **#1124 headless claude CLI hygiene** — the rerank call ran with cwd = repo root, so `claude -p` auto-discovered `CLAUDE.md` (~12k tokens per call) and burned the Max-plan window: the first S3 cloud leg fell back on 763/1000 vanilla rows and the first S2 claude leg on 237/240 rows (both discarded). Now: neutral cwd outside the repo, two-sentence `--system-prompt`, `--no-session-persistence`, UTF-8 decoding, every exception a counted fallback (`--bare` rejected: it skips the OAuth login).
- **LRB decision #2 executed — S2 re-baselined on the repaired fixture** (`2c1b210c…`, PR #1089): token V/N/J **0.2500 / 0.5875 / 0.7625**, and the four LLM-grounded legs re-run — J R@1 0.9000 / 0.9000 / 0.8625 / 1.0000 (gemma4 / gemma3:12b / mixtral / claude), V<N<J 5/5, J − N +0.1750 / +0.1625 / +0.1750 / +0.1750 / +0.2250 (token / 4B / 12B / 47B / claude), 0 timeouts, claude cells 0 fallbacks. Preprint §4 tables (S2 token axes, per-category R@10, 4-model cross-model) and abstract, pre-LOI materials, CLAUDE.md re-baselined; v0.4.4 release notes erratum extended. Report: `reports/research-runs/lrb-v021-s2-llm-grounded-repaired-fixture-20260912.md`.
- **#1126 quota-aware retry for the claude reranker** — the second S3 cloud leg still exhausted the Max-plan window overnight (876 / 1000, 1000 / 1000, 281 / 1000 rows fallback; discarded). A call whose answer looks like an exhausted window now sleeps 10 min and retries (12 h cap per process); each cell records `rerank_quota_wait_s`.
- **S3.2 LLM-grounded publication re-run, 3 local rerankers** (`reports/research-runs/lrb-v023b-3model-llm-grounded-publication-s32-20260913.md`): V<N<J 3/3, J R@1 **0.989 / 0.991 / 0.893** (gemma4 / gemma3:12b / mixtral), J − N **+0.170 / +0.178 / +0.139**, james temporal accuracy 0.995–1.000, **0 fallbacks in 9,000 rows**. Against the pre-S3.2 run: J +0.109 … +0.142 and the gap +0.012 … +0.036 wider — the removed ceiling, as predicted. The claude leg (with #1126) and preprint §4.7 follow.
- **S3.2 LLM-grounded publication, claude leg + 4-leg composite** (`reports/research-runs/lrb-v023b-claude-leg-publication-s32-20260914.md`): claude-haiku-4-5 0.852 / 0.833 / **0.990**, **0 fallbacks**, 6,600 s of usage-window waits on the JAMES cell (third attempt; the first two are discarded). J > N and J > V on all ten LLM-grounded cells across S2 + S3 (J − N +0.139 … +0.225); the strict V<N<J ordering breaks on this one leg (V ≈ N, 0.019) because every S3 superseding document says `Supersedes <id>` in its text (260/260; S2 0/48) and a strong reader uses it to rescue Vanilla — pre-registered composite for S3 publication recorded as **3/4 ⭐⭐ partial + attribution finding**, not 4/4. Preprint gains §4.7 (cross-scale × cross-model table, PDF rebuilt); README / SUMMARY carry the LLM-grounded S3 sentence; the future-work bullet becomes an S3.3 supersession-sentence ablation. Close handover §6 gains #8 (full-stack LRB cell, operator memo). **Roadmap Phase 5 measurement backlog is now exhausted except QVT 3-axis (server approval).**
...
**⚠️ Known discrepancy, deliberately not resolved here** *(resolved 2026-09-12: decision #2 taken as re-baseline — see `reports/research-runs/lrb-v021-s2-llm-grounded-repaired-fixture-20260912.md` and the `[Unreleased]` entry)*. `docs/release_notes_v0.4.4.md`, the preprint README and `.zenodo.json` carry LRB S2 as V/N/J = **0.225 / 0.5375 / 0.7125**. Since #1089 repaired a fixture collision the repository reproduces **0.2500 / 0.5875 / 0.7625** — V < N < J still holds and **J − N is 0.175 in both**, identical to four decimals. This release does **not** rebase the published figures; restating vs footnoting them is LRB decision #2, open for the operator since 2026-09-08. A green suite means the repository reproduces *itself*, not the paper.
