# 🚀 SEKOS 처음 시작 가이드 (10살도 따라할 수 있어요)

> 이 문서는 컴퓨터 잘 모르는 분도 따라할 수 있도록 천천히 한 단계씩 설명해요.
> 막히면 이 문서의 **"막혔어요!" 섹션**을 보세요.
>
> *이름.* **SEKOS** = 제품 이름 (사이드바에 보이는 것). **JAMES** =
> 그 안에서 실제로 추론하는 엔진 코드명. 같은 시스템의 두 이름이에요.

---

## 🤔 SEKOS 가 뭐예요?

SEKOS 는 **내 컴퓨터 안에서만 돌아가는 똑똑한 AI**예요.
...
SEKOS 는 보안을 위해 두 가지 비밀번호가 필요해요. **자동으로 만들어보겠습니다.**
...
이게 가장 쉬운 부분이에요. SEKOS 가 이 컴퓨터 사양을 자동으로 측정해서 적합한 모델을 추천하고 한 번 클릭하면 설치합니다.
...
1. SEKOS 화면에 **📎 클립 아이콘** 또는 **"파일 추가"** 영역 찾기
2. 본인 PDF 파일 드래그 (끌어다 놓기) 또는 클릭해서 선택
3. **업로드** 누르기
4. ⏳ SEKOS 가 PDF를 읽고 분석 (1~3분)
...
SEKOS 가 PDF 안의 정보들이 서로 어떻게 연결돼 있는지 그림으로 보여줘요.
...
**축하합니다! 🎉 SEKOS 를 처음 돌려본 분이라면, v0.3 의 라이브 사용자가 된 거예요!**
