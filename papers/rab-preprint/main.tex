...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Scenario-system co-design risk.} Scenario-S1's canonical event vocabulary (\texttt{INGEST}, \texttt{UPDATE}, \texttt{SUPERSEDE}, \texttt{DELETE}) is isomorphic to JAMES's own lifecycle taxonomy. JAMES's $1.000\times3$ on this scenario is therefore part-by-design and part-by-measurement: the gap to Baseline-0 is informative regardless, but the \emph{within-class} equality (JAMES = Reference) cannot, on a single same-vocabulary scenario, rule out unconscious shaping of the spec by the SUT it most closely matches. Three independent mitigations are queued as Future Work (§\ref{sec:future}): a Baseline-1 SUT that maps an industry tracing standard (OpenTelemetry GenAI~\cite{otel2024genaisemconv}) onto Spec~§1, scenario-S2 with a different event-vocabulary basis, and external replication invites to ActiveGraph~\cite{nakajima2026activegraph} and other audit-native runtimes whose taxonomies were defined independently of JAMES's.
...
Four explicit tracks are queued but intentionally out of scope for v0.1.1:
\begin{enumerate}[leftmargin=*]
  \item \textbf{Baseline-1: bolt-on tracing.} A SUT adapter that maps OpenTelemetry GenAI semantic conventions~\cite{otel2024genaisemconv} (and the LangSmith and MLflow trace shapes that follow them) onto Spec~§1. The mapping table itself is the deliverable that bridges audit-native scoring to industry-standard instrumentation; the measurement quantifies how far bolt-on tracing closes the gap Table~\ref{tab:gap} reports between the audit-native column and the default-logging floor. This is the highest single-step lift in scientific value v0.1.1 leaves open.
  \item \textbf{Scenario-S2 and beyond.} A larger graph (so the RF-cost column, currently negligible, becomes a quantitative axis of comparison), a multilingual corpus (so the canonical-event vocabulary is no longer single-language), and an adversarial-input track. Each scenario unlocks one step of the honest-tier ceiling and weakens the scenario-system co-design concern in §\ref{sec:limits}.
